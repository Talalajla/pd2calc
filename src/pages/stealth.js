    ...
    setCategory = () => {
        this.state.data.forEach(item => {
            ...
            if (this.state.category === "1") {
                ...
                if(item.name.toLowerCase().includes(this.state.textToMatch) && (item.category === "stealth" ))
                    filtered.push(item)}
    ...
    render() {
        ...
        return(
            <Main>
                ...
                <HeistGallery>
                    { this.state.filteredData.map((heist) => <Heist key={heist.id} name={heist.name} subname={heist.subname} img={heist.img} link={heist.link} category={heist.category} /> ) }
                </HeistGallery>
            </Main>
        );
    }
